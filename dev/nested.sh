...
ln -sfn "$ROOT/codenotch@codenotch-gnome" "$OUT/data/gnome-shell/extensions/codenotch@codenotch-gnome"
cat > "$OUT/config/glib-2.0/settings/keyfile" <<KEYS
[org/gnome/shell]
enabled-extensions=['codenotch@codenotch-gnome']
...
if [ -n "${NESTED_RELOAD:-}" ]; then
    export CODENOTCH_SHOT_LATE=6000 GSETTINGS_SCHEMA_DIR="$ROOT/codenotch@codenotch-gnome/schemas"
